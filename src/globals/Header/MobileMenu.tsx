  ...
  return (
    <div className="lg:hidden flex items-center">
      <button
        ...
        aria-label={t('home')} // Or another key like 'menu'
      >
      ...
      <div
        className={`absolute top-full left-0 right-0 z-40 bg-brand-white flex flex-col h-[calc(100vh-5rem)] pt-8 px-6 lg:px-16 overflow-y-auto duration-500 transition-all ease-[cubic-bezier(0.22,1,0.36,1)] ${
          isOpen
      ...
      >
        <nav className="flex flex-col gap-4 mb-10">
          {navLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className={`text-lg font-bold py-3 border-b border-silver-lt flex items-center justify-between transition-colors ${
                pathname === link.href ? 'text-navy-mid' : 'text-navy'
              }`}
            >
              {link.label}
            </Link>
        ...
        <div className="mt-auto pb-20 pt-10 text-center opacity-50">
          <span className="text-navy text-xs font-semibold tracking-wider">{t('topBarText')}</span>
        </div>
      </div>
    </div>
  )
