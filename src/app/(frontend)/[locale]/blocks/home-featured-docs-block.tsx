...
export default async function HomeFeaturedDocsBlock({ locale }: Props) {
    ...
    for (const doc of featured) {
        ...
        if (!grouped[key]) {
            grouped[key] = {
                category: cat ?? { id: 0, slug: 'other', title: 'Документы', order: 999, updatedAt: '', createdAt: '' } as any,
                docs: [],
            }
        }
    ...
    const orderedGroups = [
        ...categories
            .filter((cat: any) => grouped[cat.slug])
            .map((cat: any) => grouped[cat.slug]),
        ...
        <section className="bg-sky-pale px-6 lg:px-16 py-20" id="documents">
            ...
            <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between mb-12 gap-4">
                <div>
                    ...
                    <p className="mt-3 text-brand-muted text-sm leading-relaxed max-w-xl">
                        Учредительные, организационные и официальные документы Ассоциации в открытом доступе.
                    </p>
                </div>
                <Link
                    href="/documents"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                {orderedGroups.map(({ category, docs }) => (
                    ...
                    >
                        ...
                        {/* Category title */}
                        <h3 className="font-serif text-navy text-[17px] font-bold mb-3">
                            {category.title}
                        </h3>
                        ...
                        <ul className="space-y-0">
                            {docs.map((doc: any, i) => (
                                <li
                                    key={doc.id}
                                    className={i < docs.length - 1 ? 'border-b border-silver-lt' : ''}
                                >
                                    <a
                                        href={doc.url ?? '#'}
                                        target="_blank"
